...
- **`llming-com`** — the session framework. WebSocket transport,
  ...
  command-dispatch system (`SessionRouter`/`AppRouter` for reactive
  WS traffic + `@command` / `build_command_router` for HTTP), and a debug API
  that lets AI agents inspect and drive a live session.
- **`llming-stage`** — the UI foundation. App shell, SPA router,
  vendored Vue + Quasar + Tailwind + fonts + icons, and the wiring that
...
Pages, S3, any CDN. Views that need server-pushed events pull in the
WebSocket session and the full llming-com stack; views that don't, stay
static. See [Communication model](communication-model.md).
...
- **Content-hash development reload.** In development, `.vue`, `.js`,
  `.css`, `.py`, `.html`, images, and config changes are watched. The
  browser reloads only when file bytes actually change.
...
JS) and wraps it with the `$stage` client API. Without
`llming-com`:
...
See [Installation](installation.md), [Stage apps](stage.md), and
[App shell](shell.md).
